...
class GameOfLife : MonoBehaviour {
    ...
    protected void Start() {
        ...
        elapsedTime = 0f;
        CreateCell(new Vector2Int (50,50));
        for(int i = 1; i < transform.GetChild(0).childCount; i++)
        {
            grid[Mathf.RoundToInt(transform.GetChild(0).GetChild(i).position.x-0.5f), Mathf.RoundToInt(transform.GetChild(0).GetChild(i).position.y-0.5f)] = transform.GetChild(0).GetChild(i).gameObject;
        }
    }
